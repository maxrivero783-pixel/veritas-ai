...
// ==============================================================================
// Véritas v2.12 — /lib/services/spider_cloud.js
// ==============================================================================
...
async function callSearch(payload, apiKey) {
  ...
  const resp = await fetchT(`${BASE}/search`, {
    method: "POST",
    headers: {
...
async function callCrawl(payload, apiKey) {
  ...
  const resp = await fetchT(`${BASE}/crawl`, {
    method: "POST",
    headers: {
      "Authorization": `Bearer ${apiKey}`,
      "Content-Type": "application/json",
...
async function callScreenshot(payload, apiKey) {
  ...
  const resp = await fetchT(`${BASE}/screenshot`, {
    method: "POST",
    headers: {
      "Authorization": `Bearer ${apiKey}`,
      "Content-Type": "application/json",
...
async function callUnblocker(payload, apiKey) {
  ...
  const resp = await fetchT(`${BASE}/unblocker`, {
    method: "POST",
    headers: {
      "Authorization": `Bearer ${apiKey}`,
      "Content-Type": "application/json",
