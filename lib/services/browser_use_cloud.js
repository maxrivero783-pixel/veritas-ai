// ==============================================================================
// Véritas v2.4 — /lib/services/browser_use_cloud.js
// ==============================================================================
...
async function callRunTask(payload, apiKey) {
  ...
  const resp = await fetch(`${BASE}/v1/run-task`, {
    method: "POST",
    headers,
...
async function callGetTask(payload, apiKey) {
  ...
  const resp = await fetch(`${BASE}/v1/task/${task_id}`, {
    method: "GET",
    headers,
  });
  const text = await resp.text();
...
async function callSignupChallenge(payload) {
  ...
  const resp = await fetch(`${BASE}/cloud/signup/challenge`, {
    method: "POST",
...
async function callSignupVerify(payload) {
  ...
  const resp = await fetch(`${BASE}/cloud/signup/verify`, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
