...
// ==============================================================================
// Véritas v2.12 — /lib/services/llamaparse.js
// ==============================================================================
...
async function callUpload(payload, apiKey) {
  ...
  const resp = await fetchT(`${BASE}/v1/beta/files`, {
    method: "POST",
...
async function callParse(payload, apiKey) {
  ...
  const resp = await fetchT(`${BASE}/v2/parse`, {
    method: "POST",
    headers: {
...
async function callGetJob(payload, apiKey) {
  ...
  const resp = await fetchT(`${BASE}/v2/parse/${job_id}`, {
    method: "GET",
    headers: { "Authorization": `Bearer ${apiKey}` },
  });
  const text = await resp.text();
