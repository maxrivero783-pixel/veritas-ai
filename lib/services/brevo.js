...
export async function callService(env, params) {
  ...
  if (!keys.length) {
    return { ok: false, error: 'BREVO_API_KEY no configurada. Agrega BREVO_API_KEY_1 en Variables de entorno.' };
  }

  // Remitente
  const senderEmail = params.sender_email || (env && env.BREVO_SENDER_EMAIL) || null;
  const senderName  = params.sender_name  || (env && env.BREVO_SENDER_NAME) || 'Véritas AI';
