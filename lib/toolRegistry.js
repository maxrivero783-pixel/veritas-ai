// ==============================================================================
// Véritas v2.2 — /lib/toolRegistry.js (frontend)
// ==============================================================================
...
export const TOOL_REGISTRY = {
  ...
  cerebras_infer: { description: "Reservado para inferencia auxiliar Cerebras.", native: false,
    args: { prompt: { type: "string", required: true } }, allowedRoles: ["agent", "estratega", "fast"], requiresOauth: null,
  },
...
// ------------------------------------------------------------------------------
// Parser del fallback ético: <fallback_to_dolphin>...</fallback_to_dolphin>
// Devuelve { reason, original_query } o null.
// ------------------------------------------------------------------------------
export function parseFallbackToDolphin(text) {
  if (!text || typeof text !== "string") return null;
  const match = text.match(/<(fallback_to_dolphin|fallback_to_venice|fallback_to_uncensored)\s*>([\s\S]*?)<\/\1>/i);
  ...
  return {
    ...
    original_query: getTag("original_query"),
  };
}

// ------------------------------------------------------------------------------
// Elimina el bloque <fallback_to_dolphin> de una respuesta, preservando el resto.
// ------------------------------------------------------------------------------
export function stripFallbackToDolphin(text) {
  if (!text || typeof text !== "string") return "";
  return text.replace(/<(fallback_to_dolphin|fallback_to_venice|fallback_to_uncensored)\s*>[\s\S]*?<\/\1>/gi, "").trim();
}

export const parseFallbackToVenice = parseFallbackToDolphin;
export const stripFallbackToVenice = stripFallbackToDolphin;
...
export default {
  ...
  fetchAndHydrate,
  parseToolCallXML,
  parseFallbackToDolphin,
  stripFallbackToDolphin,
  parseFallbackToVenice,
  stripFallbackToVenice,
  escapeXML,
  buildToolResultXML,
};
