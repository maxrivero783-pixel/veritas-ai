...
export const TOOL_REGISTRY = {
  ...
  semantic_scholar_search: {
    ...
    allowedRoles: ["agent", "fast"], requiresOauth: null,
  },
  openalex_search: {
  ...
  open_meteo_weather: { description: "Clima actual y pronóstico con Open-Meteo.", native: false,
    args: { latitude: { type: "number", required: true }, longitude: { type: "number", required: true }, forecast_days: { type: "number", required: false, min: 1, max: 16 } }, allowedRoles: ["agent", "fast"], requiresOauth: null,
  },
  crtsh_lookup: { description: "Busca certificados y subdominios públicos en crt.sh.", native: false,
    args: { domain: { type: "string", required: true }, limit: { type: "number", required: false, min: 1, max: 100 } }, allowedRoles: ["agent", "fast"], requiresOauth: null,
  },
  rdap_lookup: { description: "Consulta RDAP público para dominios, IPs y ASNs.", native: false,
    args: { query: { type: "string", required: true }, type: { type: "string", required: false, enum: ["domain", "ip", "autnum"] } }, allowedRoles: ["agent", "fast"], requiresOauth: null,
  },
  cisa_kev_search: { description: "Busca vulnerabilidades explotadas conocidas en CISA KEV.", native: false,
    args: { query: { type: "string", required: false }, cve: { type: "string", required: false }, vendor: { type: "string", required: false }, limit: { type: "number", required: false, min: 1, max: 50 } }, allowedRoles: ["agent", "fast"], requiresOauth: null,
  },
  nvd_cve_search: { description: "Busca CVEs y detalles de severidad en NVD.", native: false,
    args: { cve: { type: "string", required: false }, keyword: { type: "string", required: false }, limit: { type: "number", required: false, min: 1, max: 20 } }, allowedRoles: ["agent", "fast"], requiresOauth: null,
  },


  // --- Recuperación v2.4: servicios y fuentes restantes ---
  geonames_search: { description: "Busca lugares con GeoNames.", native: false,
    args: { query: { type: "string", required: true }, limit: { type: "number", required: false, min: 1, max: 50 }, country: { type: "string", required: false } }, allowedRoles: ["agent", "fast"], requiresOauth: null,
  },
  nasa_search: { description: "Busca contenido público en NASA Image and Video Library.", native: false,
    args: { query: { type: "string", required: true }, media_type: { type: "string", required: false, enum: ["image", "video", "audio"] }, limit: { type: "number", required: false, min: 1, max: 20 } }, allowedRoles: ["agent", "fast"], requiresOauth: null,
  },
  sec_edgar_search: { description: "Consulta filings recientes de SEC EDGAR.", native: false,
    args: { ticker: { type: "string", required: false }, cik: { type: "string", required: false } }, allowedRoles: ["agent", "fast"], requiresOauth: null,
  },
  cohere_infer: { description: "Reservado para inferencia auxiliar Cohere.", native: false,
    args: { prompt: { type: "string", required: true } }, allowedRoles: ["agent", "fast"], requiresOauth: null,
  },
  email_report: { description: "Envía un reporte opt-in por Brevo.", native: false,
    args: { subject: { type: "string", required: false }, title: { type: "string", required: false }, summary: { type: "string", required: false }, text: { type: "string", required: false }, html: { type: "string", required: false }, consent: { type: "boolean", required: true } }, allowedRoles: ["agent", "fast"], requiresOauth: null,
  },
