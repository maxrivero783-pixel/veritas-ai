// ==============================================================================
// Véritas v2.4 — /lib/toolRegistry.js (frontend)
// ==============================================================================
...
export const TOOL_REGISTRY = {
  // --- Tools internas ---  search_repository: {
  ...
  dropbox_read_file: {
    description: "Lee un archivo de Dropbox del usuario. Extrae texto.",
    native: false,
  ...
  wikipedia_search: {
    description: "Busca contexto enciclopedico y desambiguacion en Wikipedia. Multi-idioma.",
    native: false,
  ...
  // --- OSINT publica ---
  hackernews_search: {
    description: "Busca discusiones y senales tecnicas en Hacker News via Algolia.",
    native: false,
  ...
  crtsh_lookup: {
    description: "Busca certificados SSL y subdominios publicos en crt.sh (Certificate Transparency).",
    native: false,
  ...
  rdap_lookup: {
    ...
    native: false,
    args: { query: { type: "string", required: true }, type: { type: "string", required: false, enum: ["domain", "ip", "autnum"] } },
    allowedRoles: ["agent", "strategist"],
    requiresOauth: null,
  },
  // --- Ciberseguridad ---
  cisa_kev_search: {
    description: "Busca vulnerabilidades explotadas conocidas en CISA KEV catalog.",
    native: false,
  ...
  // --- Datos publicos ---
  geonames_search: {
    description: "Busca lugares, coordenadas y poblacion con GeoNames. Requiere GEONAMES_USERNAME.",
    native: false,
  ...
  semantic_scholar_search: {
    description: "Busca papers, autores y citas en Semantic Scholar.", native: false,
    args: { query: { type: "string", required: true }, limit: { type: "number", required: false, min: 1, max: 20 } },
    allowedRoles: ["agent", "strategist", "fast"], requiresOauth: null,
  },
  openalex_search: {
    description: "Busca literatura academica abierta en OpenAlex.", native: false,
    args: { query: { type: "string", required: true }, limit: { type: "number", required: false, min: 1, max: 25 } },
    allowedRoles: ["agent", "strategist", "fast"], requiresOauth: null,
  },
  crossref_search: {
    description: "Busca metadatos bibliograficos y DOI en Crossref.", native: false,
    args: { query: { type: "string", required: true }, rows: { type: "number", required: false, min: 1, max: 20 } },
    allowedRoles: ["agent", "strategist", "fast"], requiresOauth: null,
  },
  wikipedia_search: {
    description: "Busca contexto enciclopedico en Wikipedia. Multi-idioma.", native: false,
    args: { query: { type: "string", required: true }, language: { type: "string", required: false }, limit: { type: "number", required: false, min: 1, max: 20 } },
    allowedRoles: ["agent", "strategist", "coder", "fast"], requiresOauth: null,
  },
  wikidata_search: {
    description: "Busca entidades estructuradas en Wikidata.", native: false,
    args: { query: { type: "string", required: true }, language: { type: "string", required: false }, limit: { type: "number", required: false, min: 1, max: 50 } },
    allowedRoles: ["agent", "strategist", "fast"], requiresOauth: null,
  },
  hackernews_search: {
    description: "Busca discusiones en Hacker News via Algolia.", native: false,
    args: { query: { type: "string", required: true }, tags: { type: "string", required: false }, limit: { type: "number", required: false, min: 1, max: 50 } },
    allowedRoles: ["agent", "strategist", "fast"], requiresOauth: null,
  },
  nominatim_search: {
    ...
    args: { query: { type: "string", required: true }, countrycodes: { type: "string", required: false }, limit: { type: "number", required: false, min: 1, max: 10 } },
    allowedRoles: ["agent", "strategist", "coder"], requiresOauth: null,
  },
  crtsh_lookup: {
    description: "Certificados SSL y subdominios en crt.sh.", native: false,
    args: { domain: { type: "string", required: true }, limit: { type: "number", required: false, min: 1, max: 100 } },
    allowedRoles: ["agent", "strategist"], requiresOauth: null,
  },
  rdap_lookup: {
    description: "Consulta RDAP para dominios, IPs y ASNs.", native: false,
    args: { query: { type: "string", required: true }, type: { type: "string", required: false } },
    allowedRoles: ["agent", "strategist"], requiresOauth: null,
  },
  cisa_kev_search: {
    description: "Vulnerabilidades explotadas en CISA KEV.", native: false,
    args: { cve: { type: "string", required: false }, query: { type: "string", required: false }, vendor: { type: "string", required: false }, limit: { type: "number", required: false, min: 1, max: 50 } },
    allowedRoles: ["agent", "strategist"], requiresOauth: null,
  },
  nvd_cve_search: {
    description: "CVEs y severidad en NVD (NIST).", native: false,
    args: { cve: { type: "string", required: false }, keyword: { type: "string", required: false }, limit: { type: "number", required: false, min: 1, max: 20 } },
    allowedRoles: ["agent", "strategist"], requiresOauth: null,
  },
  sec_edgar_search: {
    description: "Filings SEC EDGAR por ticker o CIK.", native: false,
    args: { ticker: { type: "string", required: false }, cik: { type: "string", required: false } },
    allowedRoles: ["agent", "strategist", "coder"], requiresOauth: null,
  },
  geonames_search: {
    description: "Lugares con GeoNames. Requiere GEONAMES_USERNAME.", native: false,
    args: { query: { type: "string", required: true }, country: { type: "string", required: false }, limit: { type: "number", required: false, min: 1, max: 50 } },
    allowedRoles: ["agent", "strategist"], requiresOauth: null,
  },
  nasa_search: {
    description: "Contenido en NASA Image and Video Library.", native: false,
    args: { query: { type: "string", required: true }, media_type: { type: "string", required: false }, limit: { type: "number", required: false, min: 1, max: 20 } },
    allowedRoles: ["agent", "coder"], requiresOauth: null,
  },
  open_meteo_weather: {
    description: "Clima y pronostico con Open-Meteo. Requiere lat/lon.", native: false,
    args: { latitude: { type: "number", required: true }, longitude: { type: "number", required: true }, forecast_days: { type: "number", required: false, min: 1, max: 16 } },
    allowedRoles: ["agent", "strategist", "coder", "fast"], requiresOauth: null,
  },
  npm_package_info: {
    description: "Metadatos de paquete npm.", native: false,
    args: { package_name: { type: "string", required: true } },
    allowedRoles: ["agent", "coder"], requiresOauth: null,
  },
  pypi_package_info: {
    description: "Metadatos de paquete PyPI.", native: false,
    args: { package_name: { type: "string", required: true } },
    allowedRoles: ["agent", "coder"], requiresOauth: null,
  },
  semantic_scholar_search: {
...
// ------------------------------------------------------------------------------
// parseFallbackToAux(text): detecta el tag de fallback asistido
//   <fallback_to_aux reason="..." original_query="...">...</fallback_to_aux>
// Compatible con el tag legacy <fallback_to_dolphin>. Devuelve null o
// { reason, original_query, prefix }.
// ------------------------------------------------------------------------------
export function parseFallbackToAux(text) {
  if (!text || typeof text !== "string") return null;
  const re = /<fallback_to_(?:aux|dolphin)\s+reason=["']([^"']*)["'](?:\s+original_query=["']([^"']*)["'])?\s*\/?>([\s\S]*?)<\/fallback_to_(?:aux|dolphin)>/i;
  const m = text.match(re);
  if (!m) return null;
  const reason = m[1] || "";
  const attrQuery = m[2] || "";
  const inner = (m[3] || "").trim();
  const original_query = attrQuery || inner;
  const idx = text.search(/<fallback_to_(?:aux|dolphin)/i);
  const prefix = idx > 0 ? text.slice(0, idx).trim() : "";
  return { reason, original_query, prefix };
}

// ------------------------------------------------------------------------------
// stripFallbackToAux(text): devuelve el texto previo al tag de fallback (si hay).
// ------------------------------------------------------------------------------
export function stripFallbackToAux(text) {
  if (!text || typeof text !== "string") return "";
  const idx = text.search(/<fallback_to_(?:aux|dolphin)/i);
...
// ------------------------------------------------------------------------------
// escapeXML(s): escapa < > & " ' para meter en <tool_result>.
// ------------------------------------------------------------------------------
...
// ------------------------------------------------------------------------------
// buildToolResultXML(name, status, output): construye el bloque para reinyectar.
// ------------------------------------------------------------------------------
export function buildToolResultXML(name, status, output) {
  return `<tool_result name="${name}" status="${status}">${escapeXML(output)}</tool_result>`;
}
