// ==============================================================================
// Véritas v2.12 — /lib/keyRotator.js
// ==============================================================================
...
export const SERVICE_REGISTRY = {
  // --- v2.7.3: nuevos proveedores (CourtListener, AviationStack, Exa, Scrape.do) ---
  courtlistener: {
    secretPrefix: "COURTLISTENER_API_TOKEN",
    cooldownMs: 60_000, // free: 5 req/min, 125/día
    maxRetries: 1,
    quotaEndpoint: "https://www.courtlistener.com/api/rest/v4/courts/",
    healthCheckMethod: "GET",
    healthCheckHeaders: (key) => ({ Authorization: `Token ${key}` }),
    healthCheckQuery: () => ({ page_size: "1" }),
  },
  aviationstack: {
    secretPrefix: "AVIATIONSTACK_API_KEY",
    cooldownMs: 60_000, // free: ~100 req/mes
    maxRetries: 1,
    quotaEndpoint: "https://api.aviationstack.com/v1/airlines",
    healthCheckMethod: "GET",
    healthCheckHeaders: () => ({}),
    healthCheckQuery: (key) => ({ access_key: key, limit: "1" }),
  },
  exa: {
    secretPrefix: "EXA_API_KEY",
    cooldownMs: 30_000,
    maxRetries: 1,
    quotaEndpoint: "https://api.exa.ai/search",
    healthCheckMethod: "POST",
    healthCheckHeaders: (key) => ({ "x-api-key": key, "Content-Type": "application/json" }),
    healthCheckBody: () => JSON.stringify({ query: "ping", numResults: 1 }),
  },
  scrapedo: {
    secretPrefix: "SCRAPEDO_API_TOKEN",
    cooldownMs: 30_000,
    maxRetries: 1,
    quotaEndpoint: "https://api.scrape.do/",
    healthCheckMethod: "GET",
    healthCheckHeaders: () => ({}),
    healthCheckQuery: (key) => ({ token: key, url: "https://httpbin.org/status/200" }),
  },
...
export function discoverKeys(env, serviceName) {
  ...
  for (let i = 1; i <= MAX_KEYS_PER_SERVICE; i++) {
    let val = env[`${svc.secretPrefix}_${i}`];
    // v2.12i: tolerar el nombre SIN sufijo _1 para la primera clave. Muchos
    // admins configuran CEREBRAS_API_KEY (sin _1); antes el rotador la ignoraba
    // y el proveedor aparecía como "key_pool_empty" pese a estar configurado.
    if (i === 1 && (typeof val !== "string" || val.length === 0)) {
      val = env[svc.secretPrefix];
    }
...
export async function getKey(env, serviceName) {
  ...
  for (let offset = 1; offset <= pool.length; offset++) {
    ...
    // v2.12: sin fila de estado ni memoria ⇒ clave nueva ⇒ saludable (antes
    // la precedencia de `??` devolvía false y la UI la mostraba como caída).
    const cooldownUntil = memState?.cooldownUntil ?? state?.cooldown_until ?? null;
    const healthy = memState?.healthy ?? (state ? state.healthy === 1 : true);

    if (healthy && (cooldownUntil === null || cooldownUntil <= now)) {
      healthyCandidates.push({ ...entry, state });
    } else if (cooldownUntil !== null && cooldownUntil > now) {
      cooledCandidates.push({ ...entry, cooldownUntil, state });
    } else {
      // Unhealthy sin cooldown vigente (o cooldown expirado) — utilizable.
      healthyCandidates.push({ ...entry, state });
    }
  }
  ...
  if (healthyCandidates.length > 0) {
    ...
    // v2.12: TODAS las claves en cooldown ⇒ lanzar AllKeysCooldownError con el
    // menor tiempo de espera. Antes se devolvía igualmente la clave "degraded",
    // lo que quemaba peticiones reales contra upstream ya rate-limited
    // (maxRetries+1 llamadas 429 por request) y nunca disparaba el manejo de
    // error diseñado en los callers (notificación + 503 all_keys_rate_limited).
    cooledCandidates.sort((a, b) => a.cooldownUntil - b.cooldownUntil);
    const minRetry = Math.max(0, cooledCandidates[0].cooldownUntil - now);
    throw new AllKeysCooldownError(serviceName, minRetry);
  } else {
    // Defensivo (no debería ocurrir: pool no vacío ⇒ siempre hay candidatos).
    chosen = { ...pool[0], state: null };
  }
...
export async function withKeyRotation(env, serviceName, fn) {
  ...
  for (let attempt = 1; attempt <= maxAttempts; attempt++) {
    ...
    try {
      ...
      // v2.12: solo los errores de RED (fetch rechaza con TypeError) marcan
      // cooldown y reintentan. Cualquier otro error (bug de código, parseo…)
      // se relanza al momento: antes envenenaba el pool entero con cooldowns
      // y quemaba todos los reintentos contra el mismo fallo.
    ...
    if (status >= 200 && status < 300) {
      ...
      // v2.12: clave inválida/sin créditos — cooldown largo Y rotar a la
      // siguiente clave (antes se devolvía el error sin probar el resto del
      // pool: una sola clave mala tumbaba todas las peticiones).
      await markCooldown(env, serviceName, index, svc.cooldownMs * 5, `HTTP ${status} auth/credits`);
      if (attempt === maxAttempts) {
        return { response, keyIndex: index, attempts, degraded: true };
      }
      continue;
...
export async function getPoolStatus(env, serviceName) {
  ...
  for (const { index } of pool) {
    ...
    const cooldownUntil = memState?.cooldownUntil ?? state.cooldown_until ?? null;
    // v2.12: clave sin fila de estado todavía = saludable (antes `?? true`
    // nunca aplicaba por precedencia y la UI la mostraba como caída).
    const hasState = Object.keys(state).length > 0 || !!memState;
    const healthy = memState?.healthy ?? (hasState ? state.healthy === 1 : true);
...
export async function forceHealthCheck(env, serviceName) {
  ...
  for (const { index, key } of pool) {
    ...
    try {
      ...
      // v2.7: timeout de 8s en health checks (evita congelar el Worker si el proveedor no responde).
      ...
      if (ok) {
        await markHealthy(env, serviceName, index);
      } else if (resp.status === 401 || resp.status === 402 || resp.status === 403) {
        // v2.12: clave inválida/sin créditos — cooldown real de 1 día
        // (antes: cooldownMs * 24 = 24 MINUTOS pese al comentario "1 día").
        await markCooldown(env, serviceName, index, 24 * 3600 * 1000, `health HTTP ${resp.status}`);
      } else {
