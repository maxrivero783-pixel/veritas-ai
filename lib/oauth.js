// ==============================================================================
// Véritas v2.4 — /lib/oauth.js
// ==============================================================================
// Helper común para OAuth de conexiones externas (GitHub, Dropbox, y futuros).
// Cubre:
...
async function refreshProviderToken(env, provider, refreshToken, oauthCreds) {
  // Import dinámico para evitar cargar adaptadores no usados en cada request.
  const adapter = (await import(`./services/oauth/${provider}.js`)).default;
  const result = await adapter.refreshToken({
    refreshToken,
    clientId: oauthCreds.clientId,
    clientSecret: oauthCreds.clientSecret,
  });
  // GitHub.refreshToken es no-op (los tokens no expiran); solo Dropbox llega aquí.
  return {
...
function getClientId(env, provider) {
  ...
  if (provider === "dropbox") return env.DROPBOX_OAUTH_APP_KEY;
  throw new Error(`Unknown provider: ${provider}`);
}

function getClientSecret(env, provider) {
  if (provider === "github") return env.GITHUB_OAUTH_CLIENT_SECRET;
  if (provider === "dropbox") return env.DROPBOX_OAUTH_APP_SECRET;
  throw new Error(`Unknown provider: ${provider}`);
}
...
export function rateLimitMessage(provider) {
  ...
  if (provider === "dropbox") {
    return "Dropbox rate limit alcanzado tras reintentos automáticos. " +
           "Espera unos segundos y vuelve a intentar la operación.";
  }
