// ==============================================================================
// Véritas v2.12 — /lib/skillsRegistry.js
// ==============================================================================
// Registro híbrido de Skills: estáticas (77 built-in) + customs del usuario (D1).
...
//   - agent:    TODAS las skills (77 built-in + customs)
...
/**
 * Definición estática de las 77 skills built-in de Véritas.
...
const SKILLS = [
  ...
  },
  {
    ...
    references: ["prompts/references/diplomatic-language-guide.md"],
    allowedRoles: ["agent"],
    icon: "\uD83C\uDF0D",
    color: "#3b82f6",
  },
  ...
  {
    id: "qingyan-research",
    name: "Investigación profunda estructurada",
  ...
  },
  {
    ...
    references: ["prompts/references/cultural-adaptation-guide.md", "prompts/references/diplomatic-language-guide.md"],
    allowedRoles: ["agent", "pensador", "fast"],
    icon: "\uD83C\uDF10",
    color: "#ec4899",
  },
  ...
  },
  {
    ...
    references: ["prompts/references/crisis-types.md", "prompts/references/diplomatic-language-guide.md"],
    allowedRoles: ["agent"],
    icon: "\uD83D\uDEA8",
    color: "#eab308",
  },
...
const _referenceContentCache = new Map(); // referencePath → string (contenido del .md)
...
/**
 * Carga un archivo markdown de referencia asociado a una skill estática.
 * Las referencias se inyectan solo cuando la skill está activa para evitar
 * inflar el system prompt global.
 *
 * @param {string} referencePath - ruta relativa desde la raíz pública.
 * @returns {Promise<string>} contenido markdown o cadena vacía.
 */
async function loadReferenceMdContent(referencePath) {
  if (!referencePath) return "";
  if (_referenceContentCache.has(referencePath)) return _referenceContentCache.get(referencePath);

  try {
    const resp = await fetch(`/${referencePath}`);
    if (!resp.ok) {
      console.warn(`[skillsRegistry] referencia no encontrada: ${referencePath}`);
      _referenceContentCache.set(referencePath, "");
      return "";
    }
    const content = (await resp.text()).trim();
    _referenceContentCache.set(referencePath, content);
    return content;
  } catch (e) {
    console.warn(`[skillsRegistry] Error cargando referencia ${referencePath}:`, e);
    _referenceContentCache.set(referencePath, "");
    return "";
  }
}
  ...
  try {
    ...
    _customSkills = (data.skills || []).map((s) => {
      return {
        ...
        allowedRoles: s.allowedRoles || ["agent", "pensador", "coder", "fast"],
        icon: s.icon || "\u2728",
...
function mergeCustomSkill(skill) {
  ...
  if (idx >= 0) {
    ...
    _customSkills.push({
      ...
      allowedRoles: skill.allowedRoles || ["agent", "pensador", "coder", "fast"],
      icon: skill.icon || "\u2728",
...
const ALL_ROLES = ["agent", "coder", "pensador", "fast"];
...
async function buildSkillsPromptBlock(userSettings, role, mode = "auto") {
  ...
  for (const skill of active) {
    ...
    // Referencias adicionales: se cargan bajo demanda y con truncado defensivo.
    if (skill.references && skill.references.length > 0) {
      const loadedRefs = await Promise.all(
        skill.references.map(async (refPath) => ({
          refPath,
          content: await loadReferenceMdContent(refPath),
        }))
      );
      const nonEmptyRefs = loadedRefs.filter((r) => r.content && r.content.trim());
      if (nonEmptyRefs.length > 0) {
        lines.push(`    Referencias de apoyo:`);
        for (const ref of nonEmptyRefs) {
          const filename = ref.refPath.split("/").pop();
          const content = ref.content.length > 6000
            ? `${ref.content.slice(0, 6000)}\n[referencia truncada a 6000 caracteres]`
            : ref.content;
          lines.push(`      <reference file="${filename}">`);
          for (const rLine of content.split("\n")) {
            lines.push(`        ${rLine}`);
          }
