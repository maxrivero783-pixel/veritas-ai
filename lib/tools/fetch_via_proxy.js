...
// ==============================================================================
// Véritas v2.12 — /lib/tools/fetch_via_proxy.js
// ==============================================================================
...
export async function run(args, ctx) {
  const { env } = ctx;
  const { url: targetUrl, method = "GET", headers = {} } = args;
  // v2.12g: 'body' como let — se reasigna abajo (rama Tavily). La
  // desestructuración const hacía el bundle fallar (esbuild: "Cannot assign
  // to 'body' because it is a constant") y en runtime lanzaba TypeError.
  let body = args.body;
  ...
  try {
    const resp = await fetchT(parsed.toString(), {
      method,
